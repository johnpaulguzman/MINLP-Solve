...
def SafeIdx(object, index_set):
    print("Saved {} with index {}".format(object.name, index_set), index_set in object.index_set())
...
def CS4520b(model, k, f, t): 
    return model.BSUF_kft[k,f,t-1] <= model.M * (1 - model.SETUPF_kft[k,f,t])
#model.CS4520b = Constraint(idx["k"], idx["f"], idx["t"], rule=CS4520b)

def CS4520c(model, k, f, t): 
    return model.BSUF_kft[k,f,t-1] >= model.BSUF_kft[k,f,t] - model.M * model.SETUPF_kft[k,f,t]
...
def CS4521b(model, k, w, t): 
    return model.BSUW_kwt[k,w,t-1] <= model.M * (1 - model.SETUPW_kwt[k,w,t])
#model.CS4521b = Constraint(idx["k"], idx["w"], idx["t"], rule=CS4521b)

def CS4521c(model, k, w, t): 
    return model.BSUW_kwt[k,w,t-1] >= model.BSUW_kwt[k,w,t] - model.M * model.SETUPW_kwt[k,w,t]
...
def OR3(model): 
    return -sum(model.ENDINVF_jft[j,f,t] * model.ICF_jf[j,f] \
        for t in idx["t"] for f in idx["f"] for j in idx["j"])

def OR4(model): 
    return -sum(model.ENDINVW_jwt[j,w,t] * model.ICW_jw[j,w] \
        for t in idx["t"] for w in idx["w"] for j in idx["j"])

def OR5(model): 
    return -sum(model.UTFW_jfwt[j,f,w,t] * model.TCFW_jfw[j,f,w] \
        for t in idx["t"] for j in idx["j"] for f in idx["f"] for w in idx["w"])

def OR6(model): 
    return -sum(model.UTFR_jfrt[j,f,r,t] * model.TCFR_jfr[j,f,r] \
        for t in idx["t"] for j in idx["j"] for f in idx["f"] for r in idx["r"])

def OR7(model): 
    return -sum(model.UTWR_jwrt[j,w,r,t] * model.TCWR_jwr[j,w,r] \
        for t in idx["t"] for j in idx["j"] for w in idx["w"] for r in idx["r"])

def OR8(model): 
    return -sum(model.BACKORDER_jrt[j,r,t] * model.BOC_r[r] \
        for t in idx["t"] for r in idx["r"] for j in idx["j"])

def OR9(model): 
    return -sum(model.INTROBINCURW_kw[k,w] * model.INTCOSTW_kw[k,w] \
        for w in idx["w"] for k in idx["k"])

def OR10(model): 
    return -sum(model.INTROBINCURF_kf[k,f] * model.INTCOSTF_kf[k,f] \
        for f in idx["f"] for k in idx["k"])

def OR11(model): 
    return -sum(model.SETUPW_kwt[k,w,t] * model.SETCOSTW_kw[k,w] \
        for t in idx["t"] for w in idx["w"] for k in idx["k"])

def OR12(model): 
    return -sum(model.SETUPF_kft[k,f,t] * model.SETCOSTF_kf[k,f] \
        for t in idx["t"] for f in idx["f"] for k in idx["k"])

def OR13(model): 
    return -sum(model.BUW_kwt[k,w,t] * model.BCW_kw[k,w] \
        for t in idx["t"] for w in idx["w"] for k in idx["k"])

def OR14(model): 
    return -sum(model.BUF_kft[k,f,t] * model.BCF_kf[k,f] \
        for t in idx["t"] for f in idx["f"] for k in idx["k"])
...
results = opt.solve(model, logfile="solver.log")#, options={"time_limit": 60}, tee=True)
