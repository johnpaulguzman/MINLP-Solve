...
if not hasattr(model, "alpha_jt"):
    alpha_generator = AlphaGenerator(model, idx)
    alpha_jt = alpha_generator.generate_alphas()
    print(">>Storing calculated alpha_jt parameters to model")
    model.alpha_jt = Param(idx["j"], idx["t"], initialize=alpha_jt, default=-1)
    model.alpha_jt.display()
    print(">>Storing calculated alpha_jt parameters to excel file")
...
if not hasattr(model, "SP_jt"):
    ...
    print(">>Storing calculated SP_jt parameters to model")
